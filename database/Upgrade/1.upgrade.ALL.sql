...
IF EXISTS(SElECT 1 FROM dbo.SYSOBJECTS WHERE Id = OBJECT_ID(N'N_UserBet') AND XType = N'U')
	AND NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'N_UserBet') AND name = N'IsSync')
	ALTER TABLE N_UserBet ADD IsSync INT;
GO

UPDATE N_UserBet SET IsSync=1 WHERE IsSync IS NULL;
GO
