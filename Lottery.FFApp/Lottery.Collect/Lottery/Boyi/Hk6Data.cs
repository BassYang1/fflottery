...
{
    public class Hk6Data
    {
        /// <summary>
        /// Log instance.
        /// </summary>
        protected static readonly ILog Log = LogManager.GetLogger(typeof(Hk6Data));
        ...
        private static ByLottery CurrentLottery = null;

        public static void Hk6(object code = null)
        {
            try
            {
                ...
                SysLotteryModel sysLottery = _lotteryDal.GetSysLotteryByCode("hk6");
                ...
                //如果上期开奖时间大于48小时
                //if (OpenTime.HasValue && OpenTime.Value.AddHours(48) > DateTime.Now)
                //{
                //    return;
                //}
                ...
                //记录当前期
                //ByLottery lottery = data.OrderByDescending(lt => lt.opentime).FirstOrDefault();

                //if (CurrentLottery == null || (lottery != null && Convert.ToDateTime(lottery.opentime) < Convert.ToDateTime(CurrentLottery.opentime)))
                //{
                ...
                foreach (ByLottery lot in data)
                {
                    ...
                    expect = expect.Substring(0, 4) + "-" + expect.Substring(4);

                    //存储开奖号码
                    if (_dal.Update(4006, expect, openCode, openTime, openCode))
                    {
                        OpenTime = Convert.ToDateTime(openTime); //开奖时间
                        Public.SaveLotteryData2File(4006); //存储开奖号码到JSON/XML文件
                        LotteryCheck.RunOfIssueNum(4006, expect); //处理开奖信息
                    }
                }
            }
            catch (Exception ex)
            {
                Log.ErrorFormat("香港六合彩: {0}", ex);
