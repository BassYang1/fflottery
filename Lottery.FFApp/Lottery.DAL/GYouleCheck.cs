...
namespace Lottery.DAL
{
  public static class GYouleCheck
  {
    public static void RunOfIssueNum(int LotteryId, string IssueNum)
    {
      ...
      doWord.BeginInvoke(LotteryId, IssueNum, new AsyncCallback(GYouleCheck.CallBack), (object) doWord);
    }

    public static void CallBack(IAsyncResult r)
    {
      GYouleCheck.DoWord asyncState = (GYouleCheck.DoWord) r.AsyncState;
    }
      ...
      try
      {
        ...
        if (dataTable.Rows.Count > 0)
        {
          ...
          if (LotteryDAL.GetCurRealGet(LotteryId) < Convert.ToDecimal(lotteryCheck.Rows[0]["CheckPer"]))
          {
            ...
            GYouleCheck.SetOpenListJson(LotteryId, IssueNum, list[0].tKey, DateTime.Now.ToString(), string.Concat((object) list[0].tValue));
          }
          else
          {
            string[] code20 = NumberCode.CreateCode20();
            GYouleCheck.SetOpenListJson(LotteryId, IssueNum, string.Join(",", code20), DateTime.Now.ToString(), "0");
          }
        }
        else
        {
