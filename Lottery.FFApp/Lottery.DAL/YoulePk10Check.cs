...
// Type: Lottery.DAL.YoulePk10Check
...
namespace Lottery.DAL
{
  public static class YoulePk10Check
  {
    ...
    public static void RunOfIssueNum(int LotteryId, string IssueNum)
    {
      ...
      doWord.BeginInvoke(LotteryId, IssueNum, new AsyncCallback(YoulePk10Check.CallBack), (object) doWord);
    }

    public static void CallBack(IAsyncResult r)
    {
      YoulePk10Check.DoWord asyncState = (YoulePk10Check.DoWord) r.AsyncState;
    }
        ...
        if (dataTable.Rows.Count > 0)
        {
          ...
          if (LotteryDAL.GetCurRealGet(LotteryId) < Convert.ToDecimal(lotteryCheck.Rows[0]["CheckPer"]))
          {
            ...
            do
            {
              ...
              codePk10 = NumberCode.CreateCodePk10(10);
              ...
              if (num11 > new Decimal(0))
                num1 = int32_1;
              YoulePk10Check.list.Add(new KeyValue()
              {
                tKey = codePk10,
                tValue = num11
              });
            ...
            while (num1 < int32_1);
            List<KeyValue> list = YoulePk10Check.list.OrderByDescending<KeyValue, Decimal>((Func<KeyValue, Decimal>) (a => a.tValue)).ToList<KeyValue>();
            if (new LotteryDataDAL().Exists(LotteryId, IssueNum))
              return;
            new LotteryDataDAL().AddYoule(LotteryId, IssueNum, list[0].tKey, DateTime.Now.ToString(), codePk10);
          ...
          else
          {
            ...
            if (new LotteryDataDAL().Exists(LotteryId, IssueNum))
              return;
            new LotteryDataDAL().AddYoule(LotteryId, IssueNum, codePk10, DateTime.Now.ToString(), codePk10);
            LotteryCheck.RunYouleOfIssueNum(LotteryId, IssueNum, codePk10);
            YoulePk10Check.SetOpenListJson(LotteryId);
          }
        }
        else
