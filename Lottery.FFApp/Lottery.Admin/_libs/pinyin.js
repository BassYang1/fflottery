function jcms$$Hash(_key,_value) 
{ 
	this.key = _key; /* 拼音*/ 
	this.value = _value; /* ascii码*/ 
} 
/* javascript 的自定义对象，用于存放汉字拼音数据字典*/ 
function jcms$$Dictionary() 
{ 
	this.items = []; 
	this.add = function(_key,_value) 
	{ 
	this.items[this.items.length] = new jcms$$Hash(_key,_value); 
	} 
} 
/*汉字拼音的数据字典-共396个-通过组合声母和韵母*/ 
var d = new jcms$$Dictionary(); 
d.add("a",-20319); 
d.add("ai",-20317); 
d.add("an",-20304); 
d.add("ang",-20295); 
...
/*转为大写*/ 
function jcms$$ConvertToLower(str, splitstr) 
{ 
	...
	{ 
		...
		if(i==1)
			result = jcms$$GetKey(ascCode); 
		else
			result = result + splitstr + jcms$$GetKey(ascCode); 
	} 
	return result.toLowerCase(); 
} 
/*转为大写*/ 
function jcms$$ConvertToUpper(str, splitstr) 
{ 
	if(!splitstr) splitstr = "";
	var result = "" ; 
	for (var i=1;i<=str.length;i++) 
