...
(function($) {
	/********** 省级数据 **********/
	var Object$Province =['安徽','澳门','北京','福建','甘肃','广东','广西','贵州','海南','河北','河南','黑龙江','湖北','湖南','吉林','江苏','江西','辽宁','内蒙古','宁夏','青海','山东','山西','陕西','上海','四川','台湾','天津','西藏','香港','新疆','云南','浙江','重庆','海外'];
	/********** 市级数据 **********/
	var Object$City = [
	...
	['阿根廷','埃及','爱尔兰','奥地利','奥克兰','澳大利亚','巴基斯坦','巴西','保加利亚','比利时','冰岛','朝鲜','丹麦','德国','俄罗斯','法国','菲律宾','芬兰','哥伦比亚','韩国','荷兰','加拿大','柬埔寨','喀麦隆','老挝','卢森堡','罗马尼亚','马达加斯加','马来西亚','毛里求斯','泰国','秘鲁','缅甸','墨西哥','南非','尼泊尔','挪威','葡萄牙','其它地区','日本','瑞典','瑞士','斯里兰卡','泰国','土耳其','委内瑞拉','文莱','乌克兰','西班牙','希腊','新加坡','新西兰','匈牙利','以色列','意大利','印度','印度尼西亚','英国','越南','智利']
	];
	$.fn.provincecity = function(opts) {
		opts = $.extend({
		...
		//分别获取2个下拉框
		var $sel1 = $('#'+opts.selProvince);
		var $sel2 = $('#'+opts.selCity);
		$sel1.append("<option value='"+opts.defProvinceVal+"'>"+opts.defProvinceVal+"</option>");//非要加这个，应该是jquery1.2.6的BUG
		$.each(Object$Province, function(index,data){
			$sel1.append("<option value='"+data+"'>"+data+"</option>");
		});
		//默认省级下拉
		//$sel1.attr("value", opts.defProvinceVal);//这句无效，应该是jquery1.2.6的BUG

		//默认的城市下拉
		$sel2.append("<option value='"+opts.defCityVal+"'>"+opts.defCityVal+"</option>");
		//省级联动 控制
		var index1 = "" ;
		$sel1.change(function(){
			...
			$.each(Object$City[index1-1], function(index,data){
