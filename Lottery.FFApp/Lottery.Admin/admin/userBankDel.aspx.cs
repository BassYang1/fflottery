...
// Type: Lottery.Admin.userBankDel
...
{
  public partial class userBankDel : AdminCenter
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      if (dataTable1.Rows.Count > 0)
      {
        ...
        if (string.IsNullOrEmpty(dataTable2.Rows[0]["Question"].ToString()) || string.IsNullOrEmpty(dataTable2.Rows[0]["Answer"].ToString()))
          this.FinalMessage("会员未绑定密保，不能删除！", "close.htm", 0);
        else
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
      ...
      {
        this.FinalMessage("会员安全答案不正确，不能删除！", "close.htm", 0);
      }
      else
      {
        this.doh.Reset();
        ...
        if (this.doh.GetDataTable().Rows.Count > 0)
        {
          ...
          if (this.doh.Delete("N_UserBank") > 0)
